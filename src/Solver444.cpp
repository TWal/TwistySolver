...
Solver444::Solver444(const CubeProperties& props) :
    ...
    _fCenters4(8, {0, 1, 2, 3}, phase4AllowedMoves, _props, [this](const char* perm) {
        char extPerm[24];
        Utils::unzoom<8>(24, fbCenters, perm, extPerm);
        return _props.makeCube().setCenterPerm(0, 0, extPerm);
    }, [](const Cube& cube, char* perm) {
    ...
    _highEdges4(8, phase4AllowedMoves, _props, [this](const char* perm) {
        char extPerm[24];
        Utils::unzoom<12>(24, highEdges, perm, extPerm);
        //for(int i = 0; i < 24; ++i) {
            //if(extPerm[i] == -1) {
                //extPerm[i] = i;
            //}
        ...
        Utils::zoom<12>(highEdges, cube.getNEdgePerm(0), perm);
        //char lows[12];
        //char highs[12];
        //Utils::zoom<12>(lowEdges, cube.getNEdgePerm(0), lows);
        //Utils::zoom<12>(highEdges, cube.getNEdgePerm(0), highs);
        //char lowsInverse[12];
        //for(int i = 0; i < 12; ++i) {
            //lowsInverse[(uint)lows[i]] = i;
        //}
        //for(int i = 0; i < 12; ++i) {
            //printf("%d ", lowsInverse[i]);
        //}
        //printf("\n");
        //for(int i = 0; i < 12; ++i) {
            //perm[i] = highs[(uint)lowsInverse[i]];
        //}
    }),
    _lowEdges4(8, phase4AllowedMoves, _props, [this](const char* perm) {
        char extPerm[24];
        Utils::unzoom<12>(24, lowEdges, perm, extPerm);
        return _props.makeCube().setNEdgePerm(0, extPerm);
    }, [](const Cube& cube, char* perm) {
        Utils::zoom<12>(lowEdges, cube.getNEdgePerm(0), perm);
    }),
    ...
    _phase4({{&_fCenters4, &_rCenters4, &_uCenters4, &_highEdges4, &_lowEdges4}}, phase4AllowedMoves, _props.identity()) {
}
